...
- 학생이 "2번째 절차", "3번째 블록", "첫 번째 줄"처럼
  **N번째 / 첫 번째 / 마지막** 등의 표현으로 특정 위치를 정확히 가리키는 경우,
  → targetScope = "BLOCK_RANGE" 로 설정해야 합니다.
  - 이때:
    - rangeIndexFrom = 해당 순번 (예: 2)
    - rangeIndexTo   = 해당 순번 (예: 2)
    - rangeCount     = 1
    - rangeAnchor    = null (HEAD/Tail은 "위에 있는 두 줄", "마지막 세 줄"처럼 상대적 표현에만 사용)
...
- 한 개만 지칭하는 경우:
  - 예: "2번째 절차를 왼쪽으로 바꿔줘"
    → rangeIndexFrom = 2, rangeIndexTo = 2
...
2. 반복문과 다중 명령이 섞여 있는 경우의 모호성 규칙
   ...
   - 이와 유사하게, "**A 하고, B를 N번 반복해**" 구조에서
    ...
    [강제 규칙: A 하고, B를 N번 반복해 패턴]

   - 아래와 같은 형태는 모두 "**다중 명령 + 반복**"으로 간주합니다.
     - "A 하고, B를 N번 반복해"
     - "A 한 다음에 B를 N번 반복해"
     - "먼저 A 하고 그다음에 B를 N번 반복해줘" 등

   - 이때, **반복 대상이 명시적으로 제한되지 않으면** 무조건 모호합니다.
     - 예를 들어, 다음과 같은 단어/표현이 **없으면** 항상 모호:
       - "B만 반복해", "앞으로 가기만 N번 반복해"
       - "두 번째 동작만 반복해"
       - "두 동작 모두 반복해", "A랑 B 전체를 반복해", "앞에서 한 동작까지 전부 반복해" 등

   - 즉, 학생 발화 안에
     - B만 반복인지 ("B만", "두 번째 명령만" 등),
     - A와 B 전체 반복인지 ("둘 다", "전체를", "앞에 한 동작까지 전부" 등)
     가 **직접적으로 드러나는 표현이 없다면** 다음처럼 처리해야 합니다.
     ...
     → 반드시 다음을 강제합니다.
       ...
       - ambiguityType = "LOOP_SCOPE_VAGUE"
       - ambiguityMessage 에서
         "B만 반복" / "A와 B 전체 반복" 두 해석을 모두 제시하고,
         학생에게 어느 쪽인지 물어보는 문장을 한국어로 작성합니다.
